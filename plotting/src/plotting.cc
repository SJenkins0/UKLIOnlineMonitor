...
void extract_data(std::ifstream &file, std::vector<float> &totQ,std::vector<float> &spotQ, std::vector<float> &monQ, std::vector<int> &year,std::vector<int> &month,std::vector<int> &day,std::vector<int> &hour,std::vector<int> &minute,std::vector<int> &second);
...
void setMinMax(TGraph *g1, TGraph *g2, TGraph *g3, TGraph *g4, TGraph *g5);
...
int main(int argc, char *argv[]){
  ...
  for (int inj=0; inj<5;inj++){
    ...
    extract_data(file,totQ,spotQ,monQ,year,month,day,hour,minute,second);
  ...
  /*  
  //Perform normalisation to the second point in the plot
  for(int i=0; i<5; i++){
    normalisation(totQVecVec[i], totQVecVec[i][1], errorTotVecVec[i]);
    normalisation(spotQVecVec[i], spotQVecVec[i][1], errorSpotVecVec[i]);

    if(totQ24VecVec[0].size()>1){
      normalisation(totQ24VecVec[i], totQ24VecVec[i][1], errorTot24VecVec[i]);
      normalisation(spotQ24VecVec[i], spotQ24VecVec[i][1], errorSpot24VecVec[i]);
    }
    if(totQ6VecVec[0].size()>1){
      normalisation(totQ6VecVec[i], totQ6VecVec[i][1], errorTot6VecVec[i]);
      normalisation(spotQ6VecVec[i], spotQ6VecVec[i][1], errorSpot6VecVec[i]);
    }
  }  
  */
  ...
  format_plots(spotQ1, spotQ2, spotQ3, spotQ4, spotQ5,leg2, 0, 250,"%m-%d","Spot Q (p.e)",tempNameSpot+"All time");
  ...
  format_plots(spotQ61, spotQ62, spotQ63, spotQ64, spotQ65,leg4, 0, 250,"%H:%M","Spot Q (p.e)",tempNameSpot+"Past 6h");
  ...
  format_plots(spotQ241, spotQ242, spotQ243, spotQ244, spotQ245,leg6, 0, 250,"%H:%M","Spot Q (p.e)",tempNameSpot+"Past 24h");
...
void extract_data(std::ifstream &file,std::vector<float> &totQ, std::vector<float> &spotQ, std::vector<float> &monQ,std::vector<int> &year,std::vector<int> &month,std::vector<int> &day,std::vector<int> &hour,std::vector<int> &minute,std::vector<int> &second){
  ...
  while(getline(file, line)){
    ...
    if(std::stod(entries.at(12))>1e-37)
...
void format_plots(TGraph *graph1,TGraph *graph2,TGraph *graph3,TGraph *graph4,TGraph *graph5, TLegend *leg,int min, int max, std::string timeform, std::string yaxis, std::string title){
  ...
  //setMinMax(graph1, graph2, graph3, graph4, graph5);
  graph2->Draw("Zpsame");
  graph3->Draw("Zpsame");
  graph4->Draw("Zpsame");
  graph5->Draw("Zpsame");
...
void setMinMax(TGraph *g1, TGraph *g2, TGraph *g3, TGraph *g4, TGraph *g5){
  ...
  std::vector<TGraph*> g = {g1, g2, g3, g4, g5};
  
  double min = 99999999, max = 0, tmp = 0;
  for (int ig=0; ig<g.size(); ig++){
    const long long N = g.at(ig)->GetN();
    long long ind[N];
    TMath::Sort(N, g.at(ig)->GetY(), ind, false);

    tmp = g.at(ig)->GetY()[ind[0]];
    if(tmp < min) min = tmp;

    tmp = g.at(ig)->GetY()[ind[N-1]];
    if(tmp > 1.2*g.at(ig)->GetY()[ind[N-2]]) tmp = g.at(ig)->GetY()[ind[N-2]];
    if(tmp > max) max = tmp;
  }

  g1->GetYaxis()->SetRangeUser(0.99*min, 1.02*max);
